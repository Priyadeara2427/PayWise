...
# backend/engine/decision_engine.py
"""
Decision Engine for financial obligations with partial payment consideration
and payment strategy integration (who you owe money to)
"""
...
# Import the new payment strategy analyzer
from .payment_strategy_analyzer import PaymentStrategyAnalyzer, analyze_payment_batch
...
class DecisionEngine:
    ...
    @staticmethod
    def make_decision(obligation: Obligation, cash_balance: float = 0, use_payment_strategy: bool = True) -> Decision:
        """
        Make decision for a single obligation with partial payment consideration
        
        Args:
            obligation: The obligation to decide on
            cash_balance: Current cash balance (for payment strategy)
            use_payment_strategy: If True, use new payment strategy analysis
        """
        ...
        # ============= NEW: Payment Strategy Analysis (Money You Owe) =============
        if use_payment_strategy:
            amount = obligation.amount
            due_date = obligation.due_date
            days_late = obligation.days_late
            
            analysis = PaymentStrategyAnalyzer.analyze_payment(
                cp_type, amount, due_date, days_late, cash_balance
            )
            
            # Determine action based on payment strategy analysis
            if analysis['payment_action'] == 'PAY_IMMEDIATELY':
                action = Action.PAY_IMMEDIATELY
                priority = Priority.CRITICAL
                reason = analysis['recommendation']
                suggested_terms = {
                    'payment_strategy': analysis,
                    ...
                    'priority_score': analysis.get('priority', 0),
            ...
            elif analysis['payment_action'] == 'NEGOTIATE_EXTENSION':
                action = Action.NEGOTIATE_EXTENSION
                priority = Priority.HIGH
                reason = analysis['recommendation']
                suggested_terms = {
                    ...
                    'can_partial': analysis.get('can_partial', False),
                    'grace_days': analysis.get('grace_days', 0),
                    'requested_days': min(analysis.get('grace_days', 15), 30),
                    'reason': 'cash_flow_management'
                }
                return Decision(
                    priority=priority,
            ...
            elif analysis['payment_action'] == 'COMMUNICATE_AND_DELAY':
                action = Action.COMMUNICATE
                priority = Priority.MEDIUM
                reason = analysis['recommendation']
                suggested_terms = {
                    'payment_strategy': analysis,
                    'message': analysis.get('message_template', ''),
                    'suggested_delay': analysis.get('grace_days', 30)
                }
                return Decision(
                    priority=priority,
                    action=action,
            ...
            elif analysis['can_partial'] and partial_terms.accepts_partial:
                action = Action.PAY_PARTIALLY
                priority = Priority.MEDIUM
                reason = f"Partial payment available. {analysis['recommendation']}"
                suggested_terms = {
                    'payment_strategy': analysis,
                    'percentage': partial_terms.suggested_pct,
                    'amount': obligation.amount * partial_terms.suggested_pct / 100,
        ...
        # ============= OLD LOGIC: Keep existing decision logic =============
    ...
    @staticmethod
    def make_decisions(obligations: List[Obligation], cash_balance: float = 0, use_payment_strategy: bool = True) -> List[Dict[str, Any]]:
        """
        Make decisions for all obligations with partial payment details
        ...
        decisions = []
        for obligation in obligations:
            decision = DecisionEngine.make_decision(obligation, cash_balance, use_payment_strategy)
            # Add decision to obligation
            obligation.decision = decision
            ...
            # Get payment strategy analysis if available
            payment_strategy = decision.suggested_terms.get('payment_strategy', {}) if decision.suggested_terms else {}
            ...
            decisions.append({
                ...
                "partial_suggestion": partial_suggestion,
                "payment_history": [h.dict() for h in obligation.partial_payment.history] if obligation.partial_payment.history else [],
                # New fields from payment strategy
                ...
                "urgency_score": payment_strategy.get('urgency_score', 0),
...
# Legacy function for backward compatibility
def run_decision_engine(financial_state: Dict[str, Any], use_payment_strategy: bool = True) -> List[Dict[str, Any]]:
    """
    Legacy function to run decision engine on financial state
    ...
    try:
        obligations_data = financial_state.get('obligations', [])
        cash_balance = financial_state.get('cash_balance', 0)
        ...
        # Generate decisions
        return DecisionEngine.make_decisions(obligations, cash_balance, use_payment_strategy)
